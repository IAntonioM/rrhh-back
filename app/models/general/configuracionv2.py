...
class Configuracionesv2Model:
    SP_NAME = "[contenedor].[sp_configuracionesv2]"
    ACTIONS = {
        "create": 1,
        "update": 2,
        "list": 3,
    ...
    COLUMN_MAPPINGS = {
        "list": [
            'id', 'configuracion_id', 'clave', 'valor', 'subvalor', 'estado', 'fecha_registro', 'estacion_registro', 'operador_registro',
    ...
    def _execute_sp(action, params=None, fetch=False):
        ...
        try:
            ...
            query = f"EXEC {Configuracionesv2Model.SP_NAME} {', '.join(sp_params)}"
    ...
    def _map_result_to_dict(result_rows, action_type):
        ...
        column_mapping = Configuracionesv2Model.COLUMN_MAPPINGS.get(action_type, [])
    ...
    def create_configuracion(data, current_user, remote_addr):
        data = Configuracionesv2Model._prepare_audit_data(data, current_user, remote_addr)
        
        params = {
            'configuracion_id': data['configuracion_id'], 
            'clave': data['clave'], 
            'valor': data['valor'], 
            'subvalor': data['subvalor'], 
            'estado': data['estado'], 
        ...
        result = Configuracionesv2Model._execute_sp(
            Configuracionesv2Model.ACTIONS["create"], 
            params
        )
    ...
    def update_configuracion(data, current_user, remote_addr):
        ...
        result = Configuracionesv2Model._execute_sp(
            Configuracionesv2Model.ACTIONS["update"], 
            params
        )
    ...
    def get_configuraciones_filter(filtros, current_page, per_page):
        params = {
            ...
            'subvalor': filtros.get('subvalor', None),
        ...
        result = Configuracionesv2Model._execute_sp(
            Configuracionesv2Model.ACTIONS["list"], 
            params,
            fetch=True
        )
        
        return Configuracionesv2Model._map_result_to_dict(result, "list")
    ...
    def delete_configuracion(id, current_user, remote_addr):
        ...
        result = Configuracionesv2Model._execute_sp(
            Configuracionesv2Model.ACTIONS["delete"], 
            params
        )
